...
const ErdFlowPage = () => {
    ...
    const schemaQuery = useQuery({
        queryKey: ["erd", "flow", includeDBMeta],
    ...
    const moduleKeys = useMemo(() => {
        const values = new Set(entities.map((e) => e.moduleKey));
    ...
    const filteredEntities = useMemo(() => {
        ...
        return entities.filter((entity) => {
            ...
            if (collapsedModules.includes(entity.moduleKey)) {
                return false;
            }
            if (!q) return true;
    ...
    }, [collapsedModules, dbGroupFilter, entities, keyword]);
            ...
            <Paper sx={{ p: 2 }}>
                ...
                <Stack direction="row" spacing={0.75} useFlexGap flexWrap="wrap" sx={{ mt: 1 }}>
                    {moduleKeys.map((moduleKey) => {
                        const collapsed = collapsedModules.includes(moduleKey);
                        return (
                            <Chip
                                key={moduleKey}
                                size="small"
                                label={`${moduleKey}${collapsed ? " (접힘)" : ""}`}
                                color={collapsed ? "default" : "info"}
                                variant={collapsed ? "outlined" : "filled"}
                ...
                <Paper sx={{ minHeight: 0, overflow: "hidden" }}>
                    ...
                    >
                        <Panel position="top-right">
                            <Chip size="small" label="ERD Flow" color="info" />
                        </Panel>
                    ...
                    {!selectedEntity ? (
                        <Stack spacing={1.25} sx={{ mt: 1.25 }}>
                            ...
                            {viewEntities.map((entity) => (
                                <Box
                                    key={entity.name}
                                    onClick={() => setSelectedNodeId(entity.name)}
                                    sx={{
                                        border: "1px solid",
                                        borderColor: "divider",
                                        borderRadius: 1.5,
