...
> `update-server` 실행 시에도 현재 OS에 맞지 않는 스크립트는 함께 정리됩니다.

## 스크립트 한눈에 보기

모든 스크립트는 `scripts/` 폴더에 있으며, 프로젝트 루트에서 실행합니다.

| 스크립트                                           | 용도                                                        | 예시                                              |
| -------------------------------------------------- | ----------------------------------------------------------- | ------------------------------------------------- |
| [`update-server`](#script-update)                  | 바이너리 버전 업데이트 (최신 / 특정 버전)                   | `./update-server.sh latest`                       |
| [`release`](#script-release)                       | `origin/main`을 `release`로 머지 후 push (배포 트리거)      | `./release.sh start`                              |
| [`run`](#script-run)                               | 서버 실행/중지/재시작/상태(dev/start/stop/restart/status, `--force`) | `./run.sh start`                          |
...
| [`service-install`](#script-service-install)       | systemd 서비스 등록/enable/start                            | `./service-install.sh`                            |
| [`service-remove`](#script-service-remove)         | systemd 서비스 stop/disable/remove                          | `./service-remove.sh`                             |

> **Windows**: `.sh` → `.ps1`로 확장자만 바꿔 실행합니다. 일부 플래그는 `--flag` → `-Flag` 형식으로 변경됩니다 (예: `./reset-all.ps1 -Force`).  
> `service-install`, `service-remove`는 Linux 전용입니다.
...
```bash
# Linux / macOS
./update-server.sh                # 도움말 + 현재 버전 + 최신 버전 확인
...
<a id="script-release"></a>

### -0) release 브랜치 배포 트리거 (`./release.sh`)

`origin/main`을 로컬 `release`에 머지한 뒤 `origin/release`로 push합니다.

```bash
# Linux / macOS
./release.sh         # 사용법 출력
./release.sh start   # 배포 트리거 실행
```

실행 순서:

1. `git fetch --prune origin`
2. 로컬 `release` 체크아웃 (없으면 생성)
3. `origin/main` 머지
4. `origin/release` push
5. 시작 브랜치로 자동 복귀

> 워킹트리가 깨끗하지 않으면 중단됩니다.
...
| 모드      | 동작                                                                                                                                                                                                                                  |
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `dev`     | `configs/server.json.environment=development`로 맞춘 뒤, `configs/database.json.default`는 그대로 유지하고 `bin/entity-server`를 포그라운드 실행. **systemd 서비스가 등록되어 있으면 실행을 막고 안내만 출력**                          |
| `start`   | `configs/server.json.environment=production`으로 맞춘 뒤, production 환경일 때만 `configs/database.json.default=production`으로 맞추고 시작 배너(`bin/entity-server banner`)를 먼저 출력한 다음 `bin/entity-server`를 백그라운드 실행 |
| `stop`    | `./run.sh`가 관리하는 백그라운드 PID를 종료하고 pid 파일 정리                                                                                                                                                                         |
| `restart` | `stop` 후 `start`. systemd 서비스가 등록되어 있으면 `systemctl restart`로 직행                                                                                                                                                        |
| `status`  | 시작 배너를 출력하며, 저작권 위 라인에 `Status: RUNNING/STOPPED`를 삽입해 표시                                                                                                                                                        |

**`--force` 플래그** (`stop` / `restart`): 프로세스명을 따지지 않고 설정 포트를 점유한 PID를 강제 종료합니다. 다른 사용자(root) 소유 프로세스도 sudo로 종료하며, systemd 서비스면 먼저 `systemctl stop`으로 완전 중지(재기동 방지)한 뒤 잔여 프로세스를 정리합니다.

**systemd 서비스로 등록된 경우** start/stop/restart는 systemctl로 동작합니다:

| 모드      | systemd 동작                                                            |
| --------- | ----------------------------------------------------------------------- |
| `start`   | `systemctl enable`(미등록 시) + `systemctl start` — 부팅 자동시작도 켬   |
| `stop`    | `systemctl disable` + `systemctl stop` — 부팅 자동시작도 끔              |
| `restart` | `systemctl restart` 직행 — enable 상태 유지(disable 하지 않음)           |
| `dev`     | 실행을 막고 안내만 출력                                                  |

> `run.sh dev`는 더 이상 `database.json`에 `development` 그룹이 있음을 요구하지 않습니다. 현재 `default`로 지정된 DB 그룹을 그대로 사용합니다.
> `run.sh start`는 서버 환경을 `production`으로 바꾸며, 이때 `database.json`에 `production` 그룹이 있어야 `default`를 `production`으로 맞출 수 있습니다.
...
./run.sh restart
./run.sh status
./run.sh stop --force      # 포트 점유 프로세스를 이름 무관 강제 종료
./run.sh restart --force   # 포트 기준 강제 종료 후 재시작
```
...
> `--apply` / `--force` 실행 전 `./normalize-entities.sh --apply`를 자동 호출해 필수 엔티티를 먼저 생성합니다.  
> 기본: `api_keys`, `rbac_roles`, `system_audit_log` / JWT 활성 시: `account`, `account_device`, `account_login_log`.
...
전체 모드(`--entity` 미지정)에서는 **필수 엔티티가 없으면 자동 생성**합니다 (`api_keys`, `rbac_roles`, `system_audit_log` 항상 / JWT 활성 시 `account`, `account_device`, `account_login_log` / Push 활성 시 `account_device`, `push_msg`, `push_log`).  
JWT 활성 기준: `configs/auth/jwt.json`이 존재하고 `jwt.enabled=true`이며 `JWT_SECRET` 환경변수가 설정된 경우. 미설정 시 기본 3개(`api_keys`, `rbac_roles`, `system_audit_log`)만 생성됩니다.
...
| 최상위 키 순서 정규화         | `name → description → cache → index → unique → fields → ...`     |
...
| 엔티티              | 기본 생성 경로                                | 비고                                                    |
| ------------------- | --------------------------------------------- | ------------------------------------------------------- |
| `api_keys`          | `entities/System/Auth/api_keys.json`          | 항상 필수, `reset-all` 시드에 필요                      |
| `rbac_roles`        | `entities/System/Auth/rbac_roles.json`        | 항상 필수, `reset-all` 시드에 필요 (기본 역할 5개 포함) |
| `account`           | `entities/System/Auth/account.json`           | JWT 활성 시 필수                                        |
| `account_device`    | `entities/System/Auth/account_device.json`    | JWT/Push 활성 시 필수                                   |
| `account_login_log` | `entities/System/Auth/account_login_log.json` | JWT 활성 시 필수 (로그인 기록 저장)                     |
| `push_msg`          | `entities/System/Push/push_msg.json`          | Push 활성 시 필수                                       |
| `push_log`          | `entities/System/Push/push_log.json`          | Push 활성 시 필수                                       |
| `system_audit_log`  | `entities/System/system_audit_log.json`       | 항상 필수                                               |
...
> 필수 엔티티 상세 설명 → [엔티티 설정 — 필수 엔티티](../data/entity-config-guide.md#필수-엔티티)
...
<a id="script-service-install"></a>

### 7) systemd 등록 (`./service-install.sh`) — Linux 전용

| 항목        | 명령                   |
| ----------- | ---------------------- |
| 실행 (기본) | `./service-install.sh` |

> root 권한이 필요하며, root가 아니면 자동으로 `sudo`로 재실행합니다.
> 실행 사용자/그룹은 현재 사용자(`id -un` / `id -gn`)로 자동 설정되며, 등록 후 `systemctl enable`까지 수행합니다.
> 이미 등록되어 있으면 기존 `ExecStart`와 함께 안내하고 재등록(덮어쓰기)으로 진행합니다.
> 서비스명은 `configs/server.json`의 `namespace`를 기준으로 자동 생성됩니다: `<namespace>-entity-server`

<a id="script-service-remove"></a>

### 8) systemd 제거 (`./service-remove.sh`) — Linux 전용

| 항목        | 명령                  |
| ----------- | --------------------- |
| 실행 (기본) | `./service-remove.sh` |

> 인수 없이 실행하면 바로 인터랙티브 모드로 진입합니다.
> 서비스가 등록되어 있지 않으면 안내 메시지와 함께 `./service-install.sh` 등록 명령을 출력합니다.
> 제거 완료 후 재등록 명령(`./service-install.sh`)을 안내합니다.
...
- [Operations Playbook (운영 플레이북)](operations-playbook.md)
- [Prefork 벤치마크 가이드](prefork-benchmark-guide.md)
