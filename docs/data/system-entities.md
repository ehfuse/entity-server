...
6. [Push 활성 시 필수](#6-push-활성-시-필수)
7. [Storage 활성 시 필수](#7-storage-활성-시-필수)
8. [SMS 활성 시 필수](#8-sms-활성-시-필수)
9. [알림톡 활성 시 필수](#9-알림톡-활성-시-필수)
10. [검증 시점과 방법](#10-검증-시점과-방법)
11. [암호화 키 결정 규칙](#11-암호화-키-결정-규칙)
...
| `account_device`    | `System/Auth/account_device.json`         | JWT 또는 Push 활성 | 명시 안 함 (기본값) |
| `account_oauth`     | `System/Auth/account_oauth.json`          | OAuth 활성         | `false`             |
| `smtp_msg`          | `System/Email/smtp_msg.json`              | SMTP 활성          | 명시 안 함 (기본값) |
| `smtp_log`          | `System/Email/smtp_log.json`              | SMTP 활성          | `false`             |
| `push_msg`          | `System/Push/push_msg.json`               | Push 활성          | `false`             |
| `push_log`          | `System/Push/push_log.json`               | Push 활성          | `false`             |
...
| `sms_msg`           | `System/Notification/sms_msg.json`        | SMS 활성           | 명시 안 함 (기본값) |
| `sms_log`           | `System/Notification/sms_log.json`        | SMS 활성           | `false`             |
| `alimtalk_msg`      | `System/Notification/alimtalk_msg.json`   | 알림톡 활성        | 명시 안 함 (기본값) |
| `alimtalk_log`      | `System/Notification/alimtalk_log.json`   | 알림톡 활성        | `false`             |
| `friendtalk_msg`    | `System/Notification/friendtalk_msg.json` | 알림톡 활성        | 명시 안 함 (기본값) |
| `friendtalk_log`    | `System/Notification/friendtalk_log.json` | 알림톡 활성        | `false`             |
...
- **역할**: 계정별 디바이스/푸시 토큰 관리
- **주요 인덱스**: `account_seq`, `device_id` (unique), `platform`, `push_token`
- **참고**: Push 활성 시에도 동일 엔티티 사용 (JWT 또는 Push 중 하나만 활성이어도 생성)

### `account_oauth` (OAuth 활성 시 추가 필수)

- **활성 조건**: `configs/auth/oauth.json`에 provider가 1개 이상 있고, 각 provider의 `client_id`/`client_secret`이 유효하며, JWT가 활성화된 경우
- **역할**: 소셜 로그인 공급자 계정과 `account`를 매핑 (`provider + provider_id → account_seq`)
- **주요 인덱스**: `account_seq`, `provider`, `provider_id`, `status`
- **특성**: `license_scope: false`, `hard_delete: true`, `history: false`, `data_encryption: true`

---

## 5) SMTP 활성 시 필수

**활성 조건**: `configs/notification/smtp.json` 파일이 존재
...
**활성 조건**: `configs/notification/push.json`의 `fcm.enabled: true` 또는 `apns.enabled: true`

### `push_msg`

- **역할**: 푸시 트리거 엔티티 (insert 시 push hook로 비동기 발송)
- **주요 인덱스**: `account_seq` (required), `device_id`, `title` (required)
- **주요 필드**: `message`, `ref_entity`, `ref_seq`, `msg_data`
- **특성**: `license_scope: false`, `hard_delete: true`, `history_ttl: 30`

### `push_log`

- **역할**: 푸시 발송 이력/재시도 상태 기록
- **주요 인덱스**: `account_seq`, `device_id`, `device_seq`, `status`, `platform`, `attempt_time`
- **주요 필드**: `device_token`, `title`, `body`, `push_data`, `error_message`, `retry_count`, `sent_time`, `ref_entity`, `ref_seq`
- **특성**: `license_scope: false`, `hard_delete: true`, `history_ttl: 30`
...
| `is_public`     | `boolean`, default `false`, index_only         | 공개 파일 여부 (인증 없이 GET 접근용) |
...
> 암호화 키 결정 규칙은 [11절](#11-암호화-키-결정-규칙)을 참조하세요.
...
**활성 조건**: `configs/notification/sms.json` 존재 + `enabled: true`

### `sms_msg`

- **역할**: SMS 발송 트리거 엔티티 — insert 시 SMS hook으로 비동기 발송
- **주요 인덱스**: `receiver` (required), `status` (`[queued, sent, failed]`), `msg_type` (`[sms, lms]`)
- **주요 필드**: `sender`, `subject`, `content`, `provider`, `ref_entity`, `ref_seq`
- **특성**: `read_only: true`, `history: false`, `hard_delete: true`, `data_encryption: true`
- **Hook**: `after_insert` → `type: sms`

### `sms_log`

- **역할**: SMS 발송 이력 — 발송 결과 추적 및 재시도 관리
- **주요 인덱스**: `status` (`[pending, processing, sent, failed, expired]`), `receiver`, `msg_type`, `sms_msg_seq`
- **주요 필드**: `provider`, `provider_msg_id`, `error_message`, `retry_count`, `sent_at`
- **특성**: `read_only: true`, `history: false`

---

## 9) 알림톡 활성 시 필수

**활성 조건**: `configs/notification/alimtalk.json` 존재 + `enabled: true`

> 알림톡과 친구톡(FriendTalk)은 동일한 알림톡 서비스를 사용하므로 4개 엔티티 모두 필수입니다.

### `alimtalk_msg`

- **역할**: 알림톡 발송 트리거 엔티티 — insert 시 alimtalk hook으로 비동기 발송
- **주요 인덱스**: `receiver` (required), `template_code` (required), `status` (`[pending, processing, sent, failed]`)
- **주요 필드**: `variables_json`, `provider`, `ref_entity`, `ref_seq`
- **특성**: `history: false`
- **Hook**: `after_insert` → `type: alimtalk`

### `alimtalk_log`

- **역할**: 알림톡 발송 이력
- **주요 인덱스**: `status`, `template_code`, `receiver`, `alimtalk_msg_seq`
- **주요 필드**: `template_name`, `variables_json`, `provider`, `provider_msg_id`, `error_message`, `retry_count`, `sent_at`
- **특성**: `read_only: true`, `history: false`

### `friendtalk_msg`

- **역할**: 친구톡 발송 트리거 엔티티 — insert 시 friendtalk hook으로 비동기 발송
- **주요 인덱스**: `receiver` (required), `msg_type` (`[text, image, wide_image, wide_item_list, carousel]`), `status`
- **주요 필드**: `content` (required), `image_url`, `image_link`, `is_ad`, `buttons_json`, `carousel_json`, `items_json`, `header`
- **특성**: `history: false`
- **Hook**: `after_insert` → `type: friendtalk`

### `friendtalk_log`

- **역할**: 친구톡 발송 이력
- **주요 인덱스**: `status`, `msg_type`, `receiver`, `friendtalk_msg_seq`
- **주요 필드**: `provider`, `provider_msg_id`, `error_message`, `retry_count`, `sent_at`
- **특성**: `read_only: true`, `history: false`
...
SMTP 활성 판정:
  configs/notification/smtp.json 존재 + enabled≠false
  → smtp_msg, smtp_log 추가

Push 활성 판정:
  configs/notification/push.json 존재 + (fcm.enabled=true 또는 apns.enabled=true)
  → account_device, push_msg, push_log 추가

OAuth 활성 판정:
  configs/auth/oauth.json 존재 + providers 1개 이상 + 각 provider client_id/client_secret 유효 + JWT 활성
  → account_oauth 추가
...
SMS 활성 판정:
  configs/notification/sms.json 존재 + enabled≠false
  → sms_msg, sms_log 추가

알림톡 활성 판정:
  configs/notification/alimtalk.json 존재 + enabled≠false
  → alimtalk_msg, alimtalk_log, friendtalk_msg, friendtalk_log 추가
```
...
| `push_msg`          | `false`         | `ENCRYPTION_KEY` (마스터)     |
| `push_log`          | `false`         | `ENCRYPTION_KEY` (마스터)     |
