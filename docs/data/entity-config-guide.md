...
엔티티 데이터는 **자유 형식 JSON**입니다. `index`·`fields` 설정은 DB 컬럼이나 필드를 강제로 생성하지 않으며, submit 시 검증·색인용 힌트 역할만 합니다.

| 기능          | 설명                                                                     |
| ------------- | ------------------------------------------------------------------------ |
| Data 저장     | **자유 형식 JSON blob** (암호화) — `fields`에 없는 키도 그대로 저장됨    |
| Index 저장    | submit 시 `index` 필드 값이 있으면 인덱스 테이블에 저장, **없으면 스킵** |
| History 저장  | 액션별 `data_snapshot` 저장                                              |
| List 필터     | index 테이블 기반 조건 검색                                              |
| `fields` 검증 | submit 데이터에 해당 키가 **있을 경우에만** 타입·enum·필수 여부를 검사   |

> **핵심**: 엔티티에 어떤 JSON 키를 저장할지는 클라이언트가 자유롭게 결정합니다. `index`와 `fields`는 그 중에서 색인하거나 검증하고 싶은 필드를 선언하는 선택적 힌트입니다.
...
`index`도 마찬가지입니다. `required: true` / `unique: true`가 아닌 `index` 필드는 submit 시 값을 보내지 않아도 무방합니다. 값이 있으면 인덱스 테이블에 기록되고, 없으면 기록하지 않을 뿐입니다.
...
| `index`의 non-required 필드를 submit에 미포함 | 인덱스 컬럼에 기록 안 됨 — **에러 아님**                 |
