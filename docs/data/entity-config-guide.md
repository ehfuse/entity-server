...
| 키         | 설명                                                                                                                 |
| ---------- | -------------------------------------------------------------------------------------------------------------------- |
| `index`    | `true`면 인덱스 필드로 동작 (정렬/필터/정확 비교 대상). idx 테이블이 canonical source 이므로 data blob 에는 저장되지 않음 |
| `comment`  | 필드 코멘트                                                                                                          |
...
| `hash`     | `true`면 인덱스 값 평문 대신 해시 저장 (`index: true` 전용). `search: true`와 함께 사용할 수 없음                    |
| `search`   | `true`면 검색 토큰 테이블에 HMAC 토큰을 저장. 원문 복원은 불가하며 LIKE/키워드 검색용. `hash: true`와 함께 사용할 수 없음 |
| `encrypt`  | `true`면 `<field>_encrypt` idx 컬럼에 암호화 저장. blob 없이 index-only 조회에서 복호 표시 가능                      |
| `no_store` | `true`면 data blob에 저장하지 않고 훅 컨텍스트에만 전달 (data 필드 전용)                                             |
| `path`     | data blob 내 중첩 경로를 dot 표기로 지정 (`index: true` 전용)                                                        |
| `where`    | 배열 와일드카드(`*`) 경로에서 대상 요소를 필터링하는 조건 객체                                                       |

### idx 필드는 자동으로 blob 에서 제외됨

모든 `index: true` 필드는 **인덱스 테이블(`entity_idx_*`)에만 저장**되며, 암호화된 data blob 에는 포함되지 않습니다. idx 테이블이 해당 필드의 canonical source 입니다.
...
```json
{
    ...
    "fields": {
        ...
        "status": { "index": true, "default": "active" },
        "rssi": { "index": true, "type": "int" }
    }
}
```
...
- **Submit/Update**: idx 필드는 인덱스 테이블에만 저장되고, data blob 암호화에서 제외됩니다.
- **Get/List**: 응답에 idx 필드가 자동으로 병합됩니다 (인덱스 테이블에서 조회).
- **History**: 비동기 history 기록 시 `idx_snapshot` + `blob_snapshot` 두 스냅샷을 함께 저장하여 완전한 revision 을 보장합니다.
- **Partial update**: idx 필드만 패치하면 ES 가 idx-only fast path 로 처리해 blob 복호/재암호화를 생략합니다.
...
> **참고**: 이전 버전의 `index_only` 키는 폐기되었습니다. 모든 idx 필드가 자동으로 동일한 동작을 하므로 별도 키가 필요하지 않습니다. 설정에 `index_only` 키가 남아 있으면 검증 오류가 발생합니다.

### search (검색 토큰)

`search: true`는 원문을 저장하지 않고 검색용 HMAC 토큰만 `entity_search_*` 테이블에 저장합니다. 따라서 `search:true` 필드만으로는 고인명, 계약자명 같은 원문을 응답에 표시할 수 없습니다.

`hash: true`와 `search: true`는 같은 필드에 함께 선언할 수 없습니다. `hash`는 정확 일치 비교용 단일 SHA-256 값이고, `search`는 n-gram 키워드 검색용 토큰 집합이므로 저장 구조와 조회 의미가 다릅니다. 함께 선언하면 엔티티 설정 검증에서 오류가 발생합니다.
...
- **Submit/Update**: 필드 값을 토큰화한 뒤 HMAC 해시로 저장합니다.
- **List/Find 조건**: 해당 필드 조건은 검색 토큰 테이블을 통해 매칭합니다.
- **응답 표시**: 검색 토큰은 비가역이므로 원문 표시에는 사용할 수 없습니다.

#### `index: true`와 함께 쓰는 경우

`index: true`와 `search: true`는 함께 사용할 수 있습니다. 이 경우 idx 테이블에는 정렬/필터/정확 비교용 컬럼이 만들어지고, 검색 테이블에는 부분 검색용 토큰이 함께 저장됩니다.

다만 같은 논리 필드에 `search: true`가 있으면 List/Find 조건은 검색 토큰 경로를 우선 사용합니다. 정확 일치 조건과 정렬을 안정적으로 분리해야 하는 필드는 아래처럼 검색 전용 필드를 별도로 두고 `path`로 같은 원문을 바라보게 하는 구성을 권장합니다.
...
### encrypt (복호 가능한 idx 필드)

`encrypt: true`는 해당 필드를 idx 필드로 취급하고, 물리 컬럼명은 `<field>_encrypt`로 생성합니다. 저장값은 data blob 암호화와 같은 키 정책을 사용합니다. `license_scope` 엔티티에서는 해당 행의 `license.secret_key`로 암호화합니다.
...
- **Submit/Update**: 원문은 data blob에 중복 저장하지 않고 `<field>_encrypt` idx 컬럼에 암호화 저장합니다.
- **Get/List**: `@indexes` 같은 index-only 조회에서도 idx 값을 복호해 논리 필드명(`deceased_name`)으로 반환합니다.
- **검색/정렬/유니크**: 암호문은 랜덤 nonce를 사용하므로 비교·정렬·유니크 용도에 적합하지 않습니다. 검색이 필요하면 `search:true`를 함께 사용합니다.

#### hash/search/encrypt 선택 기준

| 목적                    | 권장 설정                        |
| ----------------------- | -------------------------------- |
| 정확 일치/중복 검사     | `hash: true`                     |
| 키워드 검색             | `search: true`                   |
| index-only 목록 표시    | `encrypt: true`                  |
| 검색도 하고 표시도 하기 | `search: true` + `encrypt: true` |

`hash: true`와 `search: true`는 함께 사용할 수 없습니다. 정확 일치 보호와 부분 검색이 모두 필요하면 해시 필드와 검색 필드를 분리해 선언합니다.
...
`path`를 사용하면 data blob 내 **중첩된 JSON 경로**에서 값을 추출하여 인덱스 필드에 저장할 수 있습니다.
`index: true` 필드에서만 사용 가능합니다.

#### dot 표기법

경로는 `.`으로 구분합니다. 배열 인덱스도 지원합니다.
...
```json
{
    ...
    "fields": {
        "name": {
            "index": true,
            "path": "basicInfo.name",
            "comment": "data.basicInfo.name 에서 추출"
        },
        "first_role": {
            "index": true,
            "path": "roles.0.name",
            "comment": "data.roles[0].name 에서 추출"
        }
    }
}
```
...
```json
{
    "basicInfo": { "name": "홍길동", "age": 30 },
    "roles": [{ "name": "개발자" }, { "name": "디자이너" }]
}
```

결과: `name` 인덱스 → `"홍길동"`, `first_role` 인덱스 → `"개발자"`

#### 와일드카드 + where (배열 필터링)

배열 경로에 `*`를 사용하고 `where` 조건으로 대상 요소를 필터링합니다.
`*`가 포함된 `path`에는 반드시 `where`를 함께 선언해야 합니다.
...
{
    "residents": [
        { "name": "김철수", "is_representative": false },
        { "name": "김영희", "is_representative": true }
    ]
}
```

결과: `representative_name` 인덱스 → `"김영희"` (where 조건에 맞는 첫 번째 요소)

#### where에서 dot 표기법

`where`의 키도 dot 표기법을 지원합니다.
...
```json
{
    "leader_name": {
        "index": true,
        "path": "members.*.name",
        "where": { "info.role": "leader" }
    }
}
```

위 조건은 `members[i].info.role === "leader"`인 요소의 `name`을 추출합니다.

#### 값 해석 우선순위

인덱스 필드의 값을 결정할 때 다음 순서로 탐색합니다:

1. **index_fields** — payload 루트의 `index_fields` 맵에 해당 필드가 있으면 그 값 사용 (AS hook에서 직접 주입)
2. **path** — 엔티티 설정의 `path` (및 `where`)로 data blob에서 추출
3. **루트 필드** — payload 루트에 같은 이름의 키가 있으면 그 값 사용

#### 검증 규칙

- `path`는 비어 있지 않은 문자열이어야 합니다.
- `*`가 포함된 `path`에는 `where`가 필수입니다.
- `where`는 `path` 없이 단독 사용할 수 없습니다.
- `*`가 없는 `path`에 `where`를 붙이면 검증 에러입니다.
- `where`는 빈 객체가 아닌, 최소 하나의 키-값 쌍이 필요합니다.

#### backfill 지원

`path`/`where`는 ES가 직접 해석하므로, `sync.sh --with-data`로 기존 data blob을 재인덱싱할 때도 자동으로 적용됩니다. AS hook의 `index_fields` 주입 없이도 중첩 데이터를 인덱싱할 수 있습니다.
...
```json
{
    "name": "notice",
    "fields": {
        "user_seq": { "index": true, "required": true },
...
| `fields`          | 인덱스 + 데이터 필드 통합 선언. 인덱스 필드는 `"index": true` 추가. 확장 키: `comment/type/default/required/nullable/unique/hash/search/encrypt/no_store`. 중첩은 `fields.<name>.fields`로 선언 |
...
| index: true 전용 | `unique`, `hash`                                                       |
| 검색 토큰 필드    | `search` (`hash`와 동시 사용 불가, `index`와 동시 사용 시 검색 조건은 토큰 경로 우선) |
| data 필드 전용   | `no_store`                                                             |

`fields` 권장 키 순서: `index` → `comment` → `type` → `default` → `required` → `nullable` → `unique` → `hash` → `search` → `encrypt`
...
해시 저장 필드는 검색 토큰과 의미가 충돌하므로 `fields.<field>.hash=true`와 `fields.<field>.search=true`를 함께 선언하면 검증 오류가 발생합니다.
...
- 값은 아래 형식만 허용합니다.
    - 문자열 축약형: `"event_seq": "calendar_events"` 또는 `"event_seq": "calendar_events.seq"`
    - 객체형 정책 지정: `"event_seq": { "ref": "calendar_events", "onDelete": "SET NULL" }`
    - 명시적 비활성화: `"parent_seq": false`
- `false`는 `*_seq` 자동 FK 추론을 의도적으로 끄되, 해당 필드는 인덱스 필드로 유지할 때 사용합니다.
- `true`, `null`, 숫자, 빈 문자열, `ref`/`entity`가 없는 객체는 엔티티 설정 검증에서 오류로 처리됩니다.
...
- 엔티티 설정에서 별도 정책이 필요 없으면 축약형 문자열을 우선 사용합니다.
- `onDelete`, `onUpdate`를 바꿔야 할 때만 객체형 FK를 사용합니다.

## AS ensure 검증

AS가 `/v1/admin/entities/batch-ensure` 또는 개별 create 경로로 엔티티 설정을 보내면 ES는 신규 엔티티와 기존 엔티티 모두에 대해 동일한 설정 검증을 수행합니다.

- 검증 오류가 있으면 해당 엔티티 결과의 `error`에 원인이 담깁니다.
- AS는 batch ensure의 `results[].error`를 시작 오류로 전파해야 하며, 잘못된 엔티티 설정을 캐시에 처리 완료로 표시하지 않아야 합니다.
- 대표적인 검증 오류는 `hash + search` 동시 선언, `fields`에 없는 `fk` 필드, 잘못된 `fk` 값 형식입니다.
- 단, 이미 존재하는 엔티티를 AS 플러그인이 확장하는 additive ensure는 데이터 필드만 추가하는 부분 설정을 허용하므로 `index: true` 필드가 없어도 됩니다.

## 필수 엔티티

서버가 내부적으로 사용하는 시스템 엔티티(인증, SMTP, 스토리지 등)는 기능 활성화 여부에 따라 필요한 엔티티가 달라집니다.
