...
| `data_snapshot`  | LONGTEXT    | 변경 시점의 데이터(블롭) 스냅샷 (암호화)            |
| `idx_snapshot`   | LONGTEXT    | 변경 시점의 인덱스 필드 스냅샷 (rollback/복원용)    |
...
> **원자성 (SQL 경로):** 트랜잭션 단위 보상 롤백은 **단일 DB 트랜잭션으로 all-or-nothing** 처리됩니다.
> 복원 대상 중 하나라도 복원할 수 없으면(예: UPDATE인데 이전 snapshot이 없음, 스냅샷 손상 등) **롤백 전체가 실패(500)하고 아무것도 변경되지 않습니다** — "반쯤 롤백된" 상태가 생기지 않습니다.
> (해당 엔티티의 history 테이블 자체가 없으면 그 엔티티는 이 트랜잭션과 무관하므로 건너뜁니다.)
> DataStore(비SQL) 경로는 cross-collection 원자성을 보장할 수 없어 best-effort로 동작하며 실패 항목을 `errors`로 보고합니다(MongoDB 제외).
...
- [시스템 엔티티 가이드](system-entities.md)## 다음 문서
