...
- `conditions`: 조회 조건 (템플릿 지원). **템플릿 바인딩은 `${new.*}`만 지원**합니다(`${old.*}` 등은 미지원 — 코드가 `new` 소스만 해석).
...
| `after_list_each` | 목록의 각 아이템마다 | 항목별 관계 데이터 주입(`entity` 훅 등) | 기본적으로 경고 로그(항목별 실행) |
...
> **webhook 실행 시점 (중요):** `after_insert` / `after_update` / `after_delete` 의 **webhook 타입 훅은 항상 DB 트랜잭션 밖에서 실행**됩니다.
> 서버 사이드 트랜잭션(`transStart`/`transCommit`) 안에서는 **커밋이 성공한 뒤** 비동기로 발사되며(롤백되면 발사되지 않음),
> 일반 단건 쓰기에서는 데이터 커밋 직후 비동기로 발사됩니다. 따라서 webhook 의 HTTP 대기가 row lock 이나 커넥션을 점유하지 않습니다.
> webhook 실패는 응답에 영향을 주지 않고 경고 로그로만 남습니다(after 단계 기준). `sql`/`procedure`/`entity` 타입 after 훅은 기존대로 동기 실행되며, 트랜잭션 안에서는 같은 트랜잭션에 묶입니다.
>
> `before_*` webhook 은 요청을 차단(gate)하는 용도이므로 기존대로 **동기**로 실행됩니다.
