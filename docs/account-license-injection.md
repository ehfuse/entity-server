...
2. **AS**: JWT 를 검증하여 `req.account` 를 구성하고, ES 요청 시 HMAC 클라이언트에 `X-Account-Seq` 헤더를 실어 보낸다. Bearer JWT 를 ES 로 직접 전달하지 않는다.
...
| 2 | 페이로드 내 `license_seq` | 클라이언트가 명시적으로 보낸 값 |
| — | 에러 | 위 모두 없으면 에러 반환 |
...
- `license_scope` 엔티티는 페이로드에 `license_seq` 를 직접 넣거나 헤더를 수동 설정해야 한다.
