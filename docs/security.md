...
| `packet_magic_len`         | int  | 4      | 패킷 앞에 붙이는 랜덤 magic 바이트 수 (최소 2)                    |
...
| `environments`             | map  | —      | 환경별 `enable_packet_encryption` / `packet_magic_len` 오버라이드 |
...
`packet_magic_len`: 패킷 앞에 붙이는 랜덤 바이트 수입니다. 클라이언트와 서버가 동일한 값을 사용해야 합니다.  
기본값 `4`, 최솟값 `2`. 값이 클수록 패킷 구조 추론이 어려워집니다.
...
- [ ] `packet_magic_len` 값 클라이언트와 동일하게 맞추기 (기본 4)
