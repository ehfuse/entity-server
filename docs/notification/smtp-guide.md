...
> `configs/smtp.json` 파일이 없으면 SMTP 기능이 비활성화됩니다. **선택적 기능**입니다.
...
| `rate_limit`            | —      | 발송 속도 제한 (0 = 무제한, 미구현 항목) |
...
| `max_connections` | `5`        | 최대 연결 수 (현재 구현에서는 참고 설정값) |
        ...
        "after_insert": [
            {
                ...
                "template_name": "backup/backup_completed",
                "template_data": "{\"session_id\": \"${new.seq}\", \"backup_type\": \"data\"}"
            }
        ]
    }
}
...
├── auth/
│   ├── 2fa_disabled.html            ← 2FA 비활성화 알림
│   ├── 2fa_recovery_regenerated.html ← 2FA 복구 코드 재생성
│   ├── 2fa_setup_complete.html       ← 2FA 설정 완료 + 복구 코드
│
├── backup/
│   ├── backup_completed.html         ← 백업 완료
│   ├── backup_failed.html            ← 백업 실패
│   └── backup_partial.html           ← 백업 부분 성공
└── storage/
    ├── storage_quota_exceeded.html    ← 용량 초과
    └── storage_quota_warning.html     ← 용량 경고
```
...
# 백업 완료 미리보기
curl http://localhost:47200/v1/smtp/template/backup/backup_completed

# 브라우저에서 직접 열기
http://localhost:47200/v1/smtp/template/storage/storage_quota_warning?usage_percent=85
```
...
| 마케팅/대량 메일     | 전용 ESP 또는 SMTP 릴레이 검토       |
| 개발/테스트          | 직접 SMTP (Mailhog, Mailtrap 등)     |

> 현재 문서 기준으로는 SMTP 릴레이 구성이 기본 경로입니다. 대량 발송이 필요하면 전용 ESP를 별도 검토하는 편이 안전합니다.
...
- [인증 가이드](../security/auth-guide.md)
- [스토리지 가이드](../extensions/storage-guide.md)
