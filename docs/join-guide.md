...
- **특정 필드 지정**: `?fields=email,status,rbac_role` - 해당 필드만 반환, 모두 인덱스 필드면 복호화 건너뛰기
- `seq`, `created_time`, `updated_time`, `license_seq`는 항상 포함
- **검증**: 존재하지 않는 필드 요청 시 에러 발생

**`fields` 미지정 시 동작 요약**:

- 반환 범위: 인덱스 선언 필드 + 기본 메타 필드(`seq`, `created_time`, `updated_time`, `license_seq`)
- 처리 방식: data 컨럼 복호화 생람 (빠름)
- 전체 필드 반환이 필요하면 `fields=*` 명시

### 예시

**엔티티 설정** (`entities/System/Auth/account.json`):
...
`entities/System/Auth/account.json`:
...
### 5. 기본적으로 인덱스 전용 모드로 성능 최적화

`fields`를 미지정하면 기본적으로 인덱스 필드만 반환하여 복호화를 건너뛰기 성능을 향상시킵니다:
...
| **필드 선택**   | `fields=*` 로 전체 반환 (기본=인덱스만) | SELECT 절                       | 전체 반환          |

**선택 가이드**:

- **단일 엔티티 전체 조회**: List API (기본)
- **단일 엔티티 인덱스만 조회**: List API (기본, 빠름)
