...
| 파일                                                               | 목적                                |
| ------------------------------------------------------------------ | ----------------------------------- |
| [`database.json`](#database-json)                                  | DB 연결/기본 그룹 (SQL + DataStore) |
| [`server.json`](#server-json)                                      | 서버 실행 정책(환경/포트/스키마)    |
| [`logging.json`](#logging-json)                                    | 로그 레벨/채널/파일/로테이션        |
| [`cache.json`](#cache-json)                                        | 전역 캐시 정책/드라이버 설정        |
| [`extensions/storage.json`](../extensions/storage-guide.md)        | 파일 스토리지 백엔드 설정           |
| [`extensions/backup.json`](../extensions/backup-guide.md)          | 데이터/파일 백업 정책               |
| **auth/**                                                          |                                     |
| [`auth/jwt.json`](../security/jwt-auth-guide.md)                   | JWT 발급/검증 정책                  |
| [`auth/oauth.json`](../extensions/social-login-guide.md)           | 소셜 로그인(OAuth 2.0) 설정         |
| [`auth/security.json`](#security-json)                             | 보안 정책(timestamp/nonce TTL)      |
| [`auth/cors.json`](../security/security.md)                        | CORS 허용 정책                      |
| [`auth/identity.json`](../security/identity-verification-guide.md) | 휴대폰 본인인증 설정                |
| [`auth/privacy_policy.json`](../security/privacy-policy-guide.md)  | 개인정보 처리방침 설정              |
| **notification/**                                                  |                                     |
| [`notification/smtp.json`](../notification/smtp-guide.md)          | 이메일 발송 설정                    |
| [`notification/sms.json`](../notification/sms-guide.md)            | 문자 발송 설정                      |
| [`notification/push.json`](../notification/push-guide.md)          | 푸시 발송 설정                      |
| [`notification/alimtalk.json`](../notification/alimtalk-guide.md)  | 알림톡/친구톡 발송 설정             |
| [`entities/**/*.json`](../data/entity-config-guide.md)             | 엔티티 메타/스키마/기본 데이터      |

<a id="database-json"></a>

## database.json

> 비SQL 드라이버(`mongodb`, `dynamodb`, `firestore`, `scylladb`, `couchdb`)를 사용할 경우,
> 이 섹션과 함께 [데이터스토어 가이드](datastore-guide.md)를 먼저 확인하세요.
...
| `groups.<name>.driver`             | `mysql` / `mariadb` / `tidb` / `postgres` / `sqlite` / `sqlserver` (SQL) 또는 `mongodb` / `dynamodb` / `firestore` / `scylladb` / `couchdb` (DataStore) |
...
- TiDB는 MySQL 프로토콜 호환이므로 `tidb`로 지정합니다 (MySQL 드라이버 사용).
- 엔티티 설정에 `db_group`이 있으면 해당 그룹으로 라우팅됩니다.
- 위 조건이 없으면 `default` 그룹으로 라우팅됩니다.
- **DataStore 드라이버** (`mongodb`, `dynamodb`, `firestore`, `scylladb`, `couchdb`)를 지정하면 SQL Dialect 대신 DataStore 경로로 전환됩니다. 자세한 설정은 [데이터스토어 가이드](datastore-guide.md)를 참조하세요.

### MSSQL (SQL Server) 설정 예시
...
내부적으로 아래 형식의 DSN이 생성됩니다:

```
sqlserver://sa:password@localhost:1433?database=entity_db
```

| 필드       | 설명                                 |
| ---------- | ------------------------------------ |
| `driver`   | `sqlserver` (필수)                   |
| `host`     | SQL Server 호스트 (기본 `localhost`) |
| `port`     | SQL Server 포트 (기본 `1433`)        |
| `user`     | SQL Server 로그인 계정               |
| `password` | 비밀번호 (`${ENV_VAR}` 형식 지원)    |
| `database` | 대상 데이터베이스 이름               |

> **참고**: MSSQL Dialect는 MERGE 구문(인덱스 upsert), `@pN` 플레이스홀더 변환, `IDENTITY` PK를 지원합니다.
...
| `driver`      | string  | 캐시 드라이버 (`memory`, `file`, `memcached`, `redis`, `sqlite`) |
...
- TCP 커넥션 풀로 연결 재사용 (풀 크기/idle 정책은 운영 환경 부하에 맞춰 조정)
...
JWT 인증을 사용할 경우(`configs/auth/jwt.json` + `jwt.enabled=true` + `JWT_SECRET`):

- `entities/System/Auth/account.json`이 필수입니다.
...
| `public_url`              | 서버 외부 공개 URL (예: `https://api.example.com`) — identity `return_url` 등 자동 파생에 사용 |
| `language`                | `ko` 또는 `en`                                                                                 |
| `environment`             | `development` / `production`                                                                   |
| `timezone`                | 응답 시간 필드(`*_time`) 변환에 사용할 타임존                                                  |
| `port`                    | 서버 포트                                                                                      |
...
- `public_url`은 외부에서 접근 가능한 서버 URL입니다. 설정 시 `auth/identity.json`의 `return_url` 등이 자동으로 파생됩니다.
- `prefork=true`이고 `prefork_processes>0`이면 해당 값으로 프로세스 수를 제한합니다.
- Prefork 적용 전 성능 검증은 [Prefork Benchmark Guide](../operations/prefork-benchmark-guide.md)를 참고하세요.
- `enable_data_encryption`, `enable_packet_encryption` 등 암호화 설정은 `security.json`에서 관리합니다. [보안](../security/security.md)를 참고하세요.
- CORS 설정은 `auth/cors.json`에서 관리합니다.

### timezone

`server.json`의 `timezone`은 응답 JSON의 시간 필드(`*_time`)를 `YYYY-MM-DD HH:MM:SS` 형식으로 변환할 때 사용됩니다.

지원 형식:

- IANA 타임존 (예: `Asia/Seoul`, `UTC`, `America/New_York`)
- UTC 오프셋 직접 지정 (예: `+0900`, `+09:00`, `-0530`, `-05:30`)

참고:

- IANA 예시 목록: [타임존 목록](timezone-list.md)
- `timezone` 미설정 또는 잘못된 값이면 서버 OS의 시스템 로컬 타임존을 사용합니다.
...
- [본인인증 가이드](../security/identity-verification-guide.md)
