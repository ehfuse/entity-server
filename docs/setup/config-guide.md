...
| **notification/**                                           |                                     |
| [`smtp.json`](../notification/smtp-guide.md)                | 이메일 발송 설정                    |
...
| `groups.<name>.maxOpenConns`       | 최대 오픈 커넥션 수 (기본: `20`, 0 이하면 20)                                                                                                           |
| `groups.<name>.maxIdleConns`       | 최대 idle 커넥션 수 (기본: `maxOpenConns / 2`, 최소 1)                                                                                                  |
| `groups.<name>.connMaxLifetimeSec` | 커넥션 최대 수명(초) (기본: `3600` = 1시간, 0 이하면 3600)                                                                                              |
...
> `drivers` 그룹 포맷에서 실제로 파싱되는 필드는 위 예시의 키들(`file.dir`, `memcached.servers`, `redis.addr/password/db/op_timeout_seconds`)뿐입니다. `sqlite` 드라이버 경로는 그룹 포맷이 아닌 평면(flat) 포맷(`{"sqlite_path": "..."}`)에서만 지정합니다.
...
| `language`                | 서버 메시지 언어: `ko`, `en`, `ja`, `zh`, `vi`, `th`, `id`, `de`, `es` (기본 `ko`)             |
