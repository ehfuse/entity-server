...
| `push`      | 푸시 발송 작업 큐 적재     | `target_user_field` | 비동기 큐 처리 · [상세](#7-push)                                 |
...
- `type` (필수): `"push"`
- `target_user_field`: 수신자 식별 필드명 (기본: `account_seq`)
...
- `target_user_field`가 `user_seq`여도 내부에서 account 매핑 후 발송됩니다.
