...
`backup.json` 하나로 **DB 백업(엔티티 스키마 + 데이터) + 파일 백업**을 일괄 관리합니다. 외부 도구(mysqldump, rsync 등)가 필요 없으며, 이미 구현된 Storage Backend(Local/S3/Azure/GCS/SFTP/Swift)를 백업 대상으로 재사용합니다.

백업은 크게 두 축입니다:

- **`database`** — DB 백업. 엔티티 정의(스키마 JSON)와 행 데이터(JSONL)를 **한 세션으로 함께** 백업합니다. `database.enabled` 하나로 둘 다 제어합니다.
- **`files`** — 업로드된 스토리지 파일 백업(실시간 미러).
...
> `configs/backup.json`이 없거나 `enabled: false`이면 백업 기능이 비활성화됩니다. 예시 파일은 `configs-example/backup.json`을 참고하세요.
>
> 별도의 `schema` 블록은 없습니다. 스키마(엔티티 JSON 정의) 백업은 `database.enabled: true`이면 매 세션마다 자동으로 함께 수행됩니다.
...
| `storage` | string | — (필수) | 백업 저장 대상 (`storage.json`의 storages 키)   |
| `prefix`  | string | `""`    | 백업 파일 저장 경로 접두어 (빈 값 허용)         |

> `storage`는 `storage.json`에 이미 구현된 Backend를 참조하므로 별도 클라우드 설정이 필요 없습니다. storage root 자체를 백업 전용 경로(예: `/mnt/backup`)로 지정한 경우 `prefix`를 비워 중복 디렉터리를 피할 수 있습니다.

#### database (DB 백업: 스키마 + 데이터)

| 필드                              | 타입     | 기본값        | 설명                                                        |
| --------------------------------- | -------- | ------------- | ----------------------------------------------------------- |
| `enabled`                         | bool     | `false`       | DB 백업 활성화 (스키마 + 데이터 함께)                       |
| `schedule`                        | string   | `"0 2 * * *"` | cron 식 (기본: 매일 02:00)                                  |
| `db_group`                        | string[] | `["default"]` | 백업 대상 DB 그룹 (`database.json` 키)                      |
| `keep_days`                       | int      | `7`           | 보존 일수 — 이보다 오래된 세션은 자동 삭제                  |
| `keep_count`                      | int      | `3`           | 나이와 무관하게 항상 보존할 최신 세션 수 (하한)             |
| `records.entities.include`        | string[] | `[]` (=전체)  | 포함할 엔티티 (빈 배열 = 전체)                              |
| `records.entities.exclude`        | string[] | `[]`          | 제외할 엔티티                                               |
| `records.entities.exclude_patterns` | string[] | `["*_log"]` | 와일드카드 제외 패턴. 비활성화하려면 `[]`로 명시            |
| `records.include_history`         | bool     | `false`       | 히스토리 데이터 포함 여부                                   |
| `records.format`                  | string   | `"jsonl"`     | 출력 형식 (`jsonl`만 지원)                                  |
| `records.compress`                | bool     | `false`       | gzip 압축                                                   |
| `records.encrypt_passphrase`      | string   | `""`          | **예약됨 — 파일 암호화 미구현.** 설정해도 백업 파일은 암호화되지 않으며 서버가 WARN 로그를 남깁니다 |
| `records.batch_size`              | int      | `500`         | 레코드 조회 배치 크기                                       |
| `records.max_concurrent_entities` | int      | `3`           | 동시 백업 엔티티 수                                         |

> **백업 파일 자체 암호화는 아직 구현되지 않았습니다.** 백업 파일은 gzip 압축(선택) 외에 추가 암호화 없이 저장됩니다. 단, 암호화 활성 엔티티의 암호화 필드는 export 시점에 이미 암호문 상태이므로 그대로 암호문으로 백업됩니다.

#### files (파일 백업)

| 필드                 | 타입   | 기본값       | 설명                                       |
| -------------------- | ------ | ------------ | ------------------------------------------ |
| `enabled`            | bool   | `false`      | 파일 백업 활성화                           |
...
별도의 `schema` 설정 블록은 **없습니다**. `database.enabled: true`이면 매 백업 세션마다 `entities/` 디렉터리의 엔티티 JSON 정의 파일이 자동으로 함께 저장되어, 복원 시 스키마까지 복구할 수 있습니다.
...
| `on_failure` | bool     | `false`   | 백업 실패 시 알림                                 |
| `channels`   | string[] | `["log"]` | `log`, `smtp`, `webhook`(**미구현** — 로그만 남김) |

### 3. 환경 변수

`storage`, `records.encrypt_passphrase` 값은 `${ENV_VAR}` 형식으로 환경 변수를 참조할 수 있습니다:

```dotenv
BACKUP_ENCRYPT_KEY=my-secret-passphrase-256bit
```

> `encrypt_passphrase`는 현재 **예약 필드**입니다(파일 암호화 미구현). 값을 설정해도 백업 파일 암호화는 수행되지 않습니다.

---

## 백업 대상 설정 예시

최상위 `storage`는 `storage.json`의 storages를 참조합니다. 이미 구현된 6종의 Backend를 모두 사용할 수 있습니다.
...
```json
{
    ...
    "schema": {
        ...
        "files": ["Employee.json", "Department.json"]
    },
    "settings": {
        "format": "jsonl",
        "compressed": true,
...
>
> `exclude_patterns`를 지정하지 않으면(null) 기본값 `["*_log"]`가 적용되어 `*_log` 엔티티가 자동 제외됩니다. 로그 엔티티도 백업하려면 `"exclude_patterns": []`로 명시하세요.
...
스키마 백업은 별도 설정 없이 **`database.enabled: true`이면 매 백업 세션마다 자동으로** `entities/` 디렉터리의 JSON 정의 파일을 함께 저장합니다.

- 복원 시 엔티티 스키마까지 함께 복구 가능 (`entity-cli restore-schema`)
- `examples/` 하위 디렉터리는 제외
- 데이터 백업과 항상 한 세션으로 묶여 수행됩니다 (스키마만 따로 백업하는 옵션은 없음)

---

## 보존 정책 (Retention)

오래된 백업 세션은 매 백업 후 자동으로 정리됩니다:
...
기본 정책은 **시간 기준**이고, 여기에 **개수 하한**이 추가로 적용됩니다:

- `keep_days` (기본: 7): 이보다 **오래된** 세션만 삭제 대상입니다. `keep_days` 이내 세션은 모두 보존됩니다. 방금 만든 세션은 나이가 0에 가까워 항상 보존됩니다.
- `keep_count` (기본: 3): **나이와 무관하게 항상 보존할 최신 세션 개수(하한)**입니다. 시계 오류나 설정 실수로 전체 세션이 삭제 대상이 되더라도 최신 N개는 절대 삭제하지 않습니다.
- 즉, `keep_days`를 초과한 세션이라도 최신 `keep_count` 개 안에 들면 보존됩니다. `keep_count`는 "이 개수만 남기고 지운다"가 아니라 안전 하한입니다.
- 세션 ID(타임스탬프)를 파싱할 수 없는 세션은 안전을 위해 삭제하지 않습니다.
- 세션 삭제 시 해당 세션의 JSONL, manifest, schema 파일이 모두 삭제됩니다.

```bash
# 특정 세션 수동 삭제 (보존 정책과 무관하게 즉시 삭제됨 — 주의)
...
`POST /v1/admin/backup/restore` 엔드포인트는 존재하지만 **아직 구현되지 않은 스텁**입니다. 호출하면 `restore is not yet implemented via admin API — use entity-cli restore` 메시지를 반환할 뿐 실제 복원은 수행하지 않습니다. 복원은 아래 **entity-cli**를 사용하세요.
...
> **주의**: `clean` 모드는 기존 데이터를 삭제합니다. `--dry-run`으로 먼저 확인하세요. (clean 모드 실제 실행 시 `RESTORE` 타이핑 확인 프롬프트가 표시됩니다)

### CLI 복원 (entity-cli)

서버가 중단된 상태에서 CLI로 직접 복원합니다. CLI는 별도 `--config` 플래그 없이 실행 파일 위치 기준 `../configs`를 자동으로 사용하므로 **`bin/` 디렉터리의 바이너리를 실행**해야 합니다.

```bash
# 백업 세션 목록 조회
entity-cli backup-list
entity-cli backup-list --limit=5

# 특정 세션 정보 확인 (manifest)
entity-cli backup-info --session=2026-02-28T020000Z

# dry-run (실제 변경 없이 건수 확인)
entity-cli restore --session=2026-02-28T020000Z --dry-run

# 전체 복원 (clean 모드 — 'RESTORE' 입력 확인 프롬프트가 뜸)
entity-cli restore --session=2026-02-28T020000Z --mode=clean

# 특정 엔티티만 복원
entity-cli restore --session=2026-02-28T020000Z \
    --entities=Employee,Department --mode=merge

# DB 그룹 지정 (기본: default)
entity-cli restore --session=2026-02-28T020000Z --db-group=analytics

# 스키마만 복원 (엔티티 JSON 정의 파일)
entity-cli restore-schema --session=2026-02-28T020000Z
```
...
4. **암호화**: 백업 파일 자체 암호화(`encrypt_passphrase`)는 **아직 미구현**입니다. 암호화 활성 엔티티의 암호화 필드는 암호문 그대로 백업되지만, 인덱스 필드 등 평문 데이터가 포함되므로 클라우드 백업 시 스토리지 측 암호화(S3 SSE 등) 사용을 권장합니다
...
| `POST /v1/admin/backup/restore` | **미구현 (스텁)** — `entity-cli restore` 사용 |
| `POST /v1/admin/backup/delete`  | 백업 세션 삭제 (보존 정책 무관, 즉시 삭제) |
...
이것만으로 매일 02:00에 전체 엔티티 데이터 + 스키마가 `/mnt/backup/backups/`에 JSONL로 백업됩니다. (`*_log` 엔티티는 기본 제외, 보존 기본값: `keep_days: 7`, `keep_count: 3`)
...
| `webhook` | **미구현** — 지정해도 실제 알림 없이 로그만 남습니다 (향후 Hook 시스템 연동 예정) | — |
...
- 수신자: `server.json`의 `default_email_domain`(또는 `DEFAULT_EMAIL_DOMAIN` 환경변수) 기반 `admin@{domain}` 주소
