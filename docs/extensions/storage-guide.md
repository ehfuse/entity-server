...
> Entity Server 파일 스토리지 설정 및 사용법
...
서버의 `configs/extensions/` 디렉터리에 `storage.json`으로 저장합니다.

> **참고:** `configs-example/extensions/storage.json.example`에 전체 옵션이 포함된 참조 템플릿이 있습니다.
...
<a id="3-2-s3-aws--minio--cloudflare-r2"></a>
...
한도는 두 단계로 정한다. **라이선스별 한도가 우선**이고, 없으면 `storage.json` 의 기본값, 둘 다 0 이면 무제한이다.

| 위치                                | 키                 | 기본값 | 설명                                                        |
| ----------------------------------- | ------------------ | ------ | ----------------------------------------------------------- |
| `license` 엔티티 (라이선스별)       | `storage_limit_gb` | `0`    | 이 라이선스의 한도 (GB, 소수 허용). 0 = 미설정(기본값 적용) |
| `configs/storage.json` `quota`      | `default_limit_gb` | `0`    | 라이선스 한도가 없을 때 쓰는 기본 한도 (GB). 0 = 무제한     |
| `configs/storage.json` `quota`      | `threshold_percent`| `80`   | 경고 알림 임계값 (%)                                        |
| `configs/storage.json` `quota`      | `notify`           | `["log"]` | 경고/초과 알림 채널 (`log`, `smtp`)                      |
...
라이선스별 한도는 `license` 레코드의 `storage_limit_gb` 를 수정하면 된다(예: `submit license {seq, storage_limit_gb: 5}`).
`license` 엔티티는 `cache.ttl_seconds` 캐시를 타므로 변경은 최대 그 시간 뒤에 업로드 검사에 반영된다.

### 7-2. 사용량 집계 — `storage_usage` 엔티티

라이선스별 사용량은 매번 합산하지 않고 **`storage_usage` 시스템 엔티티(`entities/System/Storage/storage_usage.json`)에 카운터로 유지**한다.

- `file_meta` 행이 **INSERT** 되면(업로드·교체) `used_bytes += size`, `file_count += 1`
- `file_meta` 행이 **HARD DELETE** 되면(삭제·교체된 원본·GC 정리) `used_bytes -= size`, `file_count -= 1`
- 증감은 엔티티 서비스의 Submit/Delete 안에서 SQL 상대 갱신(`used_bytes = used_bytes + delta`)으로 일어나므로 API·GC·CLI 어느 경로든 즉시 반영되고 동시 업로드에도 정확하다.
- 집계 대상은 **원본 파일 바이트**(`file_meta.size`)이며 썸네일(sm/md/lg/webp) 파생본은 포함하지 않는다. `pending`/`orphan` 상태 파일도 디스크를 차지하므로 포함한다.
- 카운터 행이 없는 라이선스(신규·배포 직후)는 첫 조회/업로드 때 `file_meta` 합계로 자동 시딩된다. 서버 부팅 시 카운터가 하나도 없으면 전량을 1회 시딩한다.

카운터 재대조(드리프트 교정 — `relicense`·백업 복원처럼 서비스 계층을 거치지 않은 변경 뒤):

```bash
entity-cli storage-usage                    # 라이선스별 사용량/한도 조회
entity-cli storage-usage --recalc           # file_meta SUM(size)/COUNT(*) 로 전량 재집계 (drift 표시)
entity-cli storage-usage --recalc --license=5
```

### 7-2-1. 사용량 조회 API

```
POST /v1/files/usage                # 인증 컨텍스트(JWT/HMAC)의 라이선스 (POST 전용 — GET 은 무인증 뷰 경로라 제공하지 않음)
POST /v1/files/usage?license_seq=5  # apiKey 인증(AS 프록시)에서 라이선스 명시
```
...
```json
{
    ...
    "data": {
        ...
        "percent": 0,
        "exceeded": false,
        "recalc_time": "2026-08-28 18:40:12"
    }
}
```

한도가 있으면 `limit_human`, `remaining_bytes`, `percent`, `exceeded` 가 채워진다.

### 7-2-2. 업로드 검사

- 업로드/교체 시 적용 한도(라이선스 → 기본값)가 0 보다 크면 `storage_usage` 카운터 + 새 파일 크기로 초과 여부를 확인한다(교체는 기존 파일 크기를 뺀 뒤 비교).
- 초과 시 **HTTP 413** 응답:

```json
{
    "ok": false,
    "error": "Storage quota exceeded: used 8.5 GB + upload 2.0 GB exceeds limit 10.0 GB (85.0% used)"
}
```
...
| --------- | ------------------------------ |
| `log`     | 서버 로그에 경고 출력          |
| `webhook` | 설정된 webhook URL로 알림 전송 |
...
> 백업 운영 절차와 정책은 [백업](backup-guide.md)를 참조하세요.
...
> 상세 키 결정 규칙은 [시스템 엔티티 — 암호화 키 결정 규칙](../data/system-entities.md#11-암호화-키-결정-규칙)을 참조하세요.
...
| 증상            | 원인                    | 해결                                                         |
| --------------- | ----------------------- | ------------------------------------------------------------ |
| 업로드 시 413   | 쿼터 또는 파일크기 초과 | `license.storage_limit_gb`·`quota.default_limit_gb`·`max_file_size_mb` 확인, `entity-cli storage-usage` 로 사용량 확인 |
