...
아래 키들은 `index.<field>`와 `fields.<field>`에서 공통으로 사용합니다. (단, `unique`, `hash`는 `index` 전용)
...
| `index`           | 검색/정렬/카운트 조건에 사용할 필드. 권장: 객체맵으로 선언하고 `comment/type/default/required/nullable/unique/hash` 설정 (예: `index.email.required=true`) |
...
서버 운영 시 자주 확인하는 엔티티들을 먼저 요약하고, 아래에 상세 설명을 제공합니다.

### 필수 엔티티 목차 (요약)

| 엔티티              | 상세 설명 링크                                 | 생성/검증 시점 요약                                                      |
| ------------------- | ---------------------------------------------- | ------------------------------------------------------------------------ |
| `api_keys`          | [바로가기](#required-entity-api-keys)          | 항상 필요, `normalize-entities` 전체 모드에서 자동 생성                  |
| `rbac_roles`        | [바로가기](#required-entity-rbac-roles)        | 항상 필요, `normalize-entities` 전체 모드에서 자동 생성                  |
| `account`           | [바로가기](#required-entity-account)           | JWT 활성 시 필요, `normalize-entities` 전체 모드에서 자동 생성           |
| `account_login_log` | [바로가기](#required-entity-account-login-log) | JWT 활성 시 필요, `normalize-entities` 전체 모드에서 자동 생성           |
| `account_device`    | [바로가기](#required-entity-account-device)    | JWT 또는 Push 활성 시 필요, `normalize-entities` 전체 모드에서 자동 생성 |
| `push_msg`          | [바로가기](#required-entity-push-msg)          | Push 활성 시 필요, `normalize-entities` 전체 모드에서 자동 생성          |
| `push_log`          | [바로가기](#required-entity-push-log)          | Push 활성 시 필요, `normalize-entities` 전체 모드에서 자동 생성          |
| `system_audit_log`  | [바로가기](#required-entity-system-audit-log)  | 항상 필요, `normalize-entities` 전체 모드에서 자동 생성                  |

### 필수 엔티티가 언제 생기고(생성) 언제 확인되는지(검증)

- `./normalize-entities.sh --apply` (전체 모드, `--entity` 미지정): 누락된 필수 엔티티 JSON을 자동 생성합니다.
- `./reset-all.sh --apply/--force`: 실행 전 `normalize-entities --apply`를 호출하므로, 위 자동 생성 규칙이 동일하게 적용됩니다.
- 서버 시작 시에는 엔티티를 자동 생성하지 않고 설정/요건을 검증합니다.
    - JWT 활성(`jwt.enabled=true` + `JWT_SECRET`)이면 `account`, `account_device`, `account_login_log`가 필요합니다.
    - Push 활성(FCM/APNS 중 하나 enabled)이면 `account_device`, `push_msg`, `push_log`가 필요합니다.
    - `system_audit_log`가 없으면 서버 시작 시 경고가 출력됩니다.

> **JWT 활성 기준**: `configs/jwt.json`이 존재하고 `jwt.enabled=true`이며 `JWT_SECRET` 환경변수가 설정된 경우입니다.  
> JWT/Push 모두 비활성 시 자동 생성 대상은 `api_keys`, `rbac_roles` 2개입니다.

<a id="required-entity-api-keys"></a>

### `api_keys`

- 경로: `entities/System/Auth/api_keys.json`
- 역할: API 키 / HMAC 시크릿 저장 및 인증 키 매핑
- 필요 조건: 항상 필요 (JWT 사용 여부와 무관)
- 생성 시점: `normalize-entities` 전체 모드에서 누락 시 자동 생성

<a id="required-entity-rbac-roles"></a>

### `rbac_roles`

- 경로: `entities/System/Auth/rbac_roles.json`
- 역할: 역할 기반 접근 제어 권한 집합 정의
- 필요 조건: 항상 필요 (JWT 사용 여부와 무관)
- 생성 시점: `normalize-entities` 전체 모드에서 누락 시 자동 생성

<a id="required-entity-account"></a>

### `account`

- 경로: `entities/System/Auth/account.json`
- 역할: JWT 로그인/인증 계정 저장
- 필요 조건: JWT 활성 시 필수
- 생성 시점: JWT 활성 상태에서 `normalize-entities` 전체 모드 실행 시 누락분 자동 생성

<a id="required-entity-account-login-log"></a>

### `account_login_log`

- 경로: `entities/System/Auth/account_login_log.json`
- 역할: 로그인 성공/실패 추적 로그
- 필요 조건: JWT 활성 시 필수
- 생성 시점: JWT 활성 상태에서 `normalize-entities` 전체 모드 실행 시 누락분 자동 생성

<a id="required-entity-system-audit-log"></a>

### `system_audit_log`

- 경로: `entities/System/system_audit_log.json`
- 역할: 서버 레벨 감사 로그 자동 기록 대상
- 필요 조건: 항상 필요
- 생성 시점: `normalize-entities` 전체 모드에서 누락 시 자동 생성

<a id="required-entity-account-device"></a>

### `account_device`

- 경로: `entities/System/Auth/account_device.json`
- 역할: 계정별 푸시 토큰/디바이스 정보 저장
- 필요 조건: JWT 활성 시 필수 + Push 활성 시 필수 (`fcm.enabled=true` 또는 `apns.enabled=true`)
- 생성 시점: JWT 또는 Push 활성 상태에서 `normalize-entities` 전체 모드 실행 시 누락분 자동 생성

<a id="required-entity-push-msg"></a>

### `push_msg`

- 경로: `entities/System/Push/push_msg.json`
- 역할: 푸시 트리거 메시지 엔티티(훅 기반 발송 입력)
- 필요 조건: Push 활성 시 필수 (`fcm.enabled=true` 또는 `apns.enabled=true`)
- 생성 시점: Push 활성 상태에서 `normalize-entities` 전체 모드 실행 시 누락분 자동 생성

<a id="required-entity-push-log"></a>

### `push_log`

- 경로: `entities/System/Push/push_log.json`
...
| 스크립트                          | 자동 생성 대상                                                                                                                                                          |
| --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `./normalize-entities.sh --apply` | `api_keys`, `rbac_roles`, `system_audit_log` + (JWT 활성 시) `account`, `account_device`, `account_login_log` + (Push 활성 시) `account_device`, `push_msg`, `push_log` |
| `./reset-all.sh --apply/--force`  | 위와 동일 (`normalize-entities.sh --apply` 자동 호출)                                                                                                                   |
