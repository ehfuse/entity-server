...
- **DataStore 드라이버** (`mongodb`, `dynamodb`, `firestore`, `scylladb`, `couchdb`)를 지정하면 SQL Dialect 대신 DataStore 경로로 전환됩니다. 자세한 설정은 [DataStore 가이드](datastore-guide.md)를 참조하세요.
...
- `enable_data_encryption`, `enable_packet_encryption` 등 암호화 설정은 `security.json`에서 관리합니다. [보안 가이드](security.md)를 참고하세요.
