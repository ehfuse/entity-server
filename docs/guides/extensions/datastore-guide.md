...
database:
    store:
        driver: firestore
        database: my-gcp-project # GCP 프로젝트 ID
```
...
| `_counters`             | 잔로엄 `seq` (RunTransaction 원자 증가) |
...
database:
    store:
        driver: scylladb # scylladb | scylla | cassandra 모두 사용 가능
        uri: 127.0.0.1 # 콤마(,) 구분 다중 구체 지원
        database: entity_server # 키스페이스 이름
```
