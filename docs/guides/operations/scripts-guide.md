...
JWT 활성 기준: `configs/jwt.json`이 존재하고 `jwt.enabled=true`이며 `JWT_SECRET` 환경변수가 설정된 경우. 미설정 시 기본 3개(`api_keys`, `rbac_roles`, `system_audit_log`)만 생성됩니다.
...
> 필수 엔티티 상세 설명 → [entity-config-guide.md — 필수 엔티티](../data/entity-config-guide.md#필수-엔티티)
