...
| 파일                                                   | 목적                             |
| ------------------------------------------------------ | -------------------------------- |
| [`database.json`](#database-json)                      | DB 연결/기본 그룹                |
| [`security.json`](#security-json)                      | 보안 정책(timestamp/nonce TTL)   |
...
- **DataStore 드라이버** (`mongodb`, `dynamodb`, `firestore`, `scylladb`, `couchdb`)를 지정하면 SQL Dialect 대신 DataStore 경로로 전환됩니다. 자세한 설정은 [DataStore 가이드](../extensions/datastore-guide.md)를 참조하세요.
...
JWT 인증을 사용할 경우(`configs/jwt.json` + `jwt.enabled=true` + `JWT_SECRET`):
...
- `enable_data_encryption`, `enable_packet_encryption` 등 암호화 설정은 `security.json`에서 관리합니다. [보안 가이드](../security/security.md)를 참고하세요.
...
- IANA 예시 목록: [timezone-list.md](../data/timezone-list.md)
