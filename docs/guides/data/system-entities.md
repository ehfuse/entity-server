...
> Entity Server가 내부적으로 사용하는 시스템 엔티티 목록과 자동 생성 조건
...
1. [개요](#1-개요)
2. [자동 생성 조건 요약](#2-자동-생성-조건-요약)
3. [항상 필수](#3-항상-필수)
4. [JWT 활성 시 필수](#4-jwt-활성-시-필수)
5. [SMTP 활성 시 필수](#5-smtp-활성-시-필수)
6. [Push 활성 시 필수](#6-push-활성-시-필수)
7. [Storage 활성 시 필수](#7-storage-활성-시-필수)
8. [자동 생성 시점과 방법](#8-자동-생성-시점과-방법)
9. [암호화 키 결정 규칙](#9-암호화-키-결정-규칙)

---

## 1) 개요

시스템 엔티티는 `entities/System/` 하위에 위치하며 서버 기능별로 자동 생성됩니다.
`normalize-entities --apply` 실행 시 활성화된 기능에 따라 누락된 JSON 파일을 자동으로 생성합니다.
...
**활성 조건**: `configs/jwt.json`의 `enabled: true` + `JWT_SECRET` 환경변수 설정
...
**활성 조건**: `configs/push.json`의 `fcm.enabled: true` 또는 `apns.enabled: true`
...
**활성 조건**: `configs/storage.json`이 존재하고 `backends`에 1개 이상의 스토리지가 설정된 경우

### `file_meta`

- **역할**: 업로드된 파일의 메타데이터 (저장 위치, 크기, 해시, 상태) 관리
- **파일 API**: [files-routes.md](../../api-routes/files-routes.md)
- **주요 인덱스** (평문):
...
| 스크립트 / 명령                   | 동작                                                          |
| --------------------------------- | ------------------------------------------------------------- |
| `./normalize-entities.sh --apply` | 전체 모드: 활성 조건에 따라 누락된 필수 엔티티 JSON 자동 생성 |
| `./reset-all.sh --apply/--force`  | 내부적으로 `normalize-entities --apply`를 먼저 호출           |
| 서버 시작                         | 자동 생성하지 않음 — 요건만 검증하고 경고/에러 출력           |
...
JWT 활성 판정:
  configs/jwt.json 존재 + enabled=true + JWT_SECRET 환경변수 비어있지 않음
  → account, account_device, account_login_log 추가

SMTP 활성 판정:
  configs/smtp.json 존재
  → smtp_msg, smtp_log 추가

Push 활성 판정:
  configs/push.json 존재 + (fcm.enabled=true 또는 apns.enabled=true)
  → account_device, push_msg, push_log 추가

Storage 활성 판정:
  configs/storage.json 존재 + backends에 1개 이상 설정
...
3. 이미 존재하면 건너뜀 (사용자 커스터마이징 보존)
4. 없으면 기본 스펙으로 `entities/{SubDir}/{name}.json` 생성

> 자동 생성된 엔티티를 사용자가 수정해도 덮어쓰지 않습니다. 삭제 후 `normalize-entities`를 다시 실행하면 기본 스펙으로 재생성됩니다.
