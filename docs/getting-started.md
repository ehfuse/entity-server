...
| #   | 단계                 | 설명                                                        | 참고 문서                                                           |
| --- | -------------------- | ----------------------------------------------------------- | ------------------------------------------------------------------- |
| 1   | **아키텍처 이해**    | 시스템 구성, 데이터 모델, 클라이언트 연동 패턴 파악         | [Architecture](architecture.md)                              |
...
| 5   | **entities 설정**    | 도메인 엔티티 JSON 파일 작성                                | [Entity Config Guide](entity-config-guide.md)                |
| 6   | **reset-all 초기화** | 전체 테이블 생성 + 기본 데이터 시드                         | [Scripts Guide](scripts-guide.md)                            |
| 7   | **DB 확인**          | 테이블·데이터 정상 생성 여부 확인                           | [Operations Playbook](operations-playbook.md)                |
| 8   | **관리자 웹 접속**   | Admin Web UI 실행 및 접속 후 엔티티 관리                    | [아래 §7](#admin-web-setup)                                         |
| 9   | **인증 설정**        | JWT / HMAC / OAuth 2.0 설정 및 인증 흐름 확인               | [Auth Guide](auth-guide.md) · [Security](security.md) |
| 10  | **API 목록**         | 엔티티 CRUD·조회·필터 API 엔드포인트 확인                   | [Entity Routes](entity-routes.md) · [API Routes](api-routes.md)     |
| 11  | **조인 방법**        | 엔티티 간 관계 및 join 쿼리 패턴                            | [Join Guide](join-guide.md)                                  |
| 12  | **버전 업데이트**    | `./scripts/update-server.sh` 로 바이너리+배포 파일 업데이트 | [아래 §8](#8-버전-업데이트)                                         |
| 13  | **운영 플레이북**    | 배포·점검·장애 대응 절차                                    | [Operations Playbook](operations-playbook.md)                |
| 14  | **스크립트 가이드**  | 전체 CLI 스크립트 옵션 및 사용 예제                         | [Scripts Guide](scripts-guide.md)                            |
...
| DB      | MySQL/MariaDB, PostgreSQL, SQLite, SQL Server |
...
## 7-1) 클라이언트 SDK 연동 (`entity-server-client`)
