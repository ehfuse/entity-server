...
서명 대상 문자열(string to sign) — **파이프(`|`) 구분, raw body 그대로**:
...
| 항목        | 값                     | 설명                                                                                        |
| ----------- | ---------------------- | ------------------------------------------------------------------------------------------- |
| `METHOD`    | HTTP 메서드            | `c.Method()` 그대로 (`GET`, `POST`, `PUT`, `DELETE` 등)                                     |
| `path`      | 경로 + 쿼리 문자열     | `/v1/entity/user/list?page=1` — 쿼리 포함 originalURL 우선, 폴백으로 경로만. **수신한 raw 값 그대로** |
| `timestamp` | Unix 초 (정수, 문자열) | `X-Timestamp` 헤더와 동일한 값. **초(seconds) 단위, 밀리초 아님**                           |
| `nonce`     | 1회용 난수 문자열      | `X-Nonce` 헤더와 동일. 재사용 방지를 위해 NonceStore에 저장됨(기본 TTL 300초)                |
| `body`      | **raw body 바이트 그대로** | SHA-256 해시하지 않고 **전송된 raw body**를 그대로 사용. body가 없으면 빈 문자열             |

> 세 헤더 `X-Signature`, `X-Timestamp`, `X-Nonce`가 모두 필요합니다. 하나라도 없으면 인증 실패합니다.
> body는 **해시하지 않습니다** — raw body 바이트를 그대로 문자열에 이어 붙입니다. 따라서 `multipart/form-data`는 boundary/part 순서/줄바꿈까지 포함한 **원본 raw body**가 동일해야 같은 서명이 나옵니다.
...
```
packet_key = HKDF-SHA256(access_token_string, info="entity-server:packet-encryption", len=32)
```
...
[binary: XChaCha20-Poly1305( HKDF-SHA256(access_token,"entity-server:packet-encryption"), nonce:24, {"ok":true,"data":{...}} )]
```

클라이언트 처리 순서:

1. `X-Access-Token` 헤더 읽기
2. `HKDF-SHA256(token, "entity-server:packet-encryption")`로 복호 키 파생
...
sequenceDiagram
    ...
    alt 4xx / 5xx 에러 응답
        ...
        PE->>PE: key = HKDF-SHA256(jwt_token,"…packet-encryption")<br/>XChaCha20-Poly1305 암호화
        PE-->>C: 바이너리 스트림 (Content-Type: application/octet-stream)
    end
```
...
sequenceDiagram
    ...
    PE->>PE: key = HKDF-SHA256(X-Access-Token 헤더값,"…packet-encryption")<br/>XChaCha20-Poly1305 암호화
...
└─ PacketEncrypt     : auth_method="jwt" + jwt_token 있음
                       → key = HKDF-SHA256(jwt_token, "entity-server:packet-encryption")
                       → 요청 복호 + 응답 암호화  ✅
```
...
| 버전      | magic 값          | 설명                         |
| --------- | ----------------- | ---------------------------- |
| v1 (구) | `"001"` (3바이트) | 구 포맷. 현재 코드는 복호를 지원하지 않음 |
| v2 (현재) | `"002"` (3바이트) | 현행 포맷 — HKDF-SHA256 키 파생 + XChaCha20-Poly1305. `DecryptData`는 magic `"002"`만 허용 |
