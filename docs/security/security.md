...
[Client Request]
  ...
  [Handler]
  ↓
  ③ 패킷 암호화 미들웨어   → 2xx 응답 본문 XChaCha20-Poly1305 암호화 (전송 보호)
  ↓
  [Client Response]
```
...
- **패킷 암호화**: 응답 본문을 클라이언트별 키로 암호화 (전송 중 탈취 방지)

세 레이어는 독립적으로 활성화/비활성화 가능합니다.
...
| 환경 변수        | 필수 | 설명                                               |
| ---------------- | ---- | -------------------------------------------------- |
| `ENCRYPTION_KEY` | ✅   | 데이터 암/복호화 키 (XChaCha20-Poly1305, 32바이트) |
...
## 설정 파일: `configs/auth/security.json`
...
| `enable_packet_encryption` | bool | false  | API 응답 패킷 암호화 활성화 (XChaCha20-Poly1305) |
| `timestamp_skew_sec`       | int  | 300    | 타임스킬프 허용 오차 (초)                        |
...
| `environments`             | map  | —      | 환경별 `enable_packet_encryption` 오버라이드     |
...
`enable_packet_encryption: true` 시 인증에 성공한 **2xx 응답 본문**을 XChaCha20-Poly1305로 암호화합니다.  
4xx/5xx 오류 응답은 평문 JSON 그대로 전송됩니다.

> **admin 라우트 제외** — `/v1/admin/` 경로는 패킷 암호화 대상에서 항상 제외됩니다.  
> admin-web은 서버 내부망 전용 관리 도구이므로 plain JSON으로 통신합니다.  
> HMAC 서명 + RBAC 역할 인가가 이미 적용되어 있어 추가 암호화 없이도 충분히 안전합니다.
...
| HMAC API 키 인증 | `HKDF-SHA256(hmac_secret, "entity-server:packet-encryption")`  |

> 클라이언트는 `Content-Type: application/octet-stream` 수신 시 동일 키로 복호화합니다.
...
> magic 바이트 길이(K)는 `K = 2 + key[31] % 14` 공식으로 패킷 키에서 자동 파생됩니다.  
> 서버와 클라이언트가 동일한 패킷 키를 보유하면 별도 설정 공유 없이 자동 일치합니다.

### 환경별 오버라이드

개발 환경은 평문, 프로덕션만 암호화하려면 `environments`를 사용합니다:
...
> `environments` 키는 `configs/server.json`의 `environment` 값과 일치할 때 오버라이드가 적용됩니다.

### DB 데이터 암호화와의 차이

| 항목                | DB 데이터 암호화                                 | 패킷 암호화                                               |
| ------------------- | ------------------------------------------------ | --------------------------------------------------------- |
| 설정 위치           | `configs/server.json` → `enable_data_encryption` | `configs/auth/security.json` → `enable_packet_encryption` |
| 엔티티별 오버라이드 | entity JSON의 `encrypt: true/false`              | 없음 (전체 응답 적용)                                     |
| 암호화 범위         | DB `data` 컬럼 (저장 시)                         | HTTP 응답 본문 (전송 시)                                  |
| 복호화 주체         | 서버 자체 (읽기 시 자동 복호화)                  | 클라이언트 (수신 후 복호화)                               |
| 키                  | `ENCRYPTION_KEY` 환경변수 기반                   | 인증 토큰/HMAC secret 기반 (요청별 동적)                  |

---

## 3단계: RBAC 역할 기반 접근 제어
...
- [ ] `enable_packet_encryption: true` 설정 (클라이언트가 복호화 지원하는 경우)
...
- [Encryption Guide (암호화 가이드)](encryption-guide.md)
