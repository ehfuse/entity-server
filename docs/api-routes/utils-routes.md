...
| 메서드 | 경로                                            | 설명                                            |
| ------ | ----------------------------------------------- | ----------------------------------------------- |
| `GET`  | [`/v1/utils/address/sido`](#address-sido)       | 시/도 전체 목록                                 |
| `GET`  | [`/v1/utils/address/sigungu`](#address-sigungu) | 시/군/구 목록 (`?sido=` 필터)                   |
| `GET`  | [`/v1/utils/address/dong`](#address-dong)       | 읍/면/동 목록 (`?sido=&sigungu=` 필터)          |
| `GET`  | [`/v1/utils/address/clean`](#address-clean)     | 자유형식 주소 → 시도·시군구·읍면동 분해 (`?q=`) |
| `GET` `POST` | [`/v1/utils/address/resolve`](#address-resolve) | 자유형식 주소 → 시도·시군구(seq 포함) 분해 (`?q=` / `?address=` / JSON body) |

### QR 코드

| 메서드       | 경로                                    | 설명                        |
| ------------ | --------------------------------------- | --------------------------- |
| `GET` `POST` | [`/v1/utils/qrcode`](#qr-png)           | QR 코드 → PNG 바이너리      |
| `GET` `POST` | [`/v1/utils/qrcode/base64`](#qr-base64) | QR 코드 → Base64 + data URI |
| `GET` `POST` | [`/v1/utils/qrcode/text`](#qr-text)     | QR 코드 → ASCII 아트        |

### 바코드

| 메서드       | 경로                                | 설명                         |
| ------------ | ----------------------------------- | ---------------------------- |
| `GET` `POST` | [`/v1/utils/barcode`](#barcode-png) | 바코드(1D/2D) → PNG 바이너리 |

### PDF 변환

| 메서드       | 경로                                     | 설명                                         |
| ------------ | ---------------------------------------- | -------------------------------------------- |
| `POST`       | [`/v1/utils/pdf2png`](#pdf2png)          | PDF 업로드 → PNG (단일: 바이너리, 복수: ZIP) |
| `GET` `POST` | [`/v1/utils/pdf2png/:fileSeq`](#pdf2png) | 스토리지 파일 → PNG                          |
| `POST`       | [`/v1/utils/pdf2jpg`](#pdf2jpg)          | PDF 업로드 → JPG (단일: 바이너리, 복수: ZIP) |
| `GET` `POST` | [`/v1/utils/pdf2jpg/:fileSeq`](#pdf2jpg) | 스토리지 파일 → JPG                          |
...
<a id="address-resolve"></a>

### GET · POST /v1/utils/address/resolve

자유 형식 한국 주소를 **시도·시군구**(각 `seq` 포함)까지 분해합니다. `clean`과 달리 읍/면/동(`dong`)은 반환하지 않으며, 시도·시군구 매칭과 그 식별자(seq)가 필요한 경우에 사용합니다. 입력은 쿼리스트링과 JSON 본문을 모두 허용합니다.

```
GET  /v1/utils/address/resolve?q=서울 강남구 역삼동 123-4
GET  /v1/utils/address/resolve?address=경기 수원시 팔달구 인계동
POST /v1/utils/address/resolve   { "address": "서울 강남구 역삼동" }
```
...
| 위치       | 파라미터                | 설명                          |
| ---------- | ----------------------- | ----------------------------- |
| JSON body  | `address` → `q`         | `Content-Type: application/json` 일 때 |
| query      | `address` → `q`         | 본문이 비어 있을 때 사용      |

> `address`와 `q` 중 하나는 필수입니다. 모두 비어 있으면 `400`.
...
시도 매칭 실패 시 `data.ok: false`로 반환됩니다.

#### 에러

| 상태 코드 | 조건                          | 응답                                                          |
| --------- | ----------------------------- | ------------------------------------------------------------- |
| `400`     | `address`/`q` 모두 누락        | `{"ok": false, "message": "address or q parameter is required"}` |
| `400`     | 잘못된 JSON 본문              | `{"ok": false, "message": "Invalid request body"}`            |
| `503`     | 엔티티 서비스 미설정          | `{"ok": false, "message": "entity service is not available"}` |
