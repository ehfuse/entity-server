...
- **[인증 라우트](auth-routes.md)** — `/v1/auth/*`, `/v1/oauth/*` 인증·계정 관리 API
- **[이메일 인증 라우트](email-verification-routes.md)** — `/v1/email/verification/*`, `/v1/email/change` 이메일 인증 및 변경 API (5개)
...
- **[푸시 라우트](push-routes.md)** — `/v1/push` 푸시 알림 API (3개)
- **[편의 기능 라우트](utils-routes.md)** — `/v1/utils` QR 코드 생성 등 편의 API
- **[PG 결제 라우트](pg-routes.md)** — `/v1/pg` 주문·결제 승인·취소·웹훅 API (7개)
- **[SMS 라우트](sms-routes.md)** — `/v1/sms` SMS/LMS/MMS 발송·인증번호 API (4개)
- **[알림톡·친구톡 라우트](alimtalk-routes.md)** — `/v1/alimtalk`, `/v1/friendtalk` 카카오 알림톡/친구톡 API (5개)
- **[본인인증 라우트](identity-routes.md)** — `/v1/identity` 본인인증 요청·결과 조회 API (4개)
