...
`seq` 는 큐에 들어간 `smtp_log` 의 seq 다 — 발송 상태 조회(`POST /v1/smtp/status/:seq`)의 키로 쓴다.
...
```json
{
    ...
    "data": {
        ...
        "provider": "main",
        "retry_count": 0,
        "created_time": "2026-09-07 13:52:35",
        "attempt_time": "2026-09-07 13:52:40",
        "sent_time": "2026-09-07 13:52:41",
...
`status` 는 `pending → processing → sent | failed` 로 진행한다. 시각은 서버 로컬 시간대의 `YYYY-MM-DD HH:MM:SS` 다.
없는 seq 는 `404`.
...
| `backup/backup_completed.html`        | 백업 완료      | `${session_id}`, `${backup_type}`, `${duration}`          |
| `backup/backup_failed.html`           | 백업 실패      | `${session_id}`, `${error_message}`                       |
| `backup/backup_partial.html`          | 백업 부분 완료 | `${session_id}`, `${failed_count}`, `${completed_count}`  |
| `storage/storage_quota_exceeded.html` | 저장공간 초과  | `${used_bytes}`, `${limit_bytes}`, `${usage_percent}`     |
| `storage/storage_quota_warning.html`  | 저장공간 경고  | `${used_bytes}`, `${limit_bytes}`, `${usage_percent}`     |
