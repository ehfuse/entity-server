...
| `push`       | 푸시 발송 작업 큐 적재     | `target_account_seq` 또는 `target_device_id`  | 비동기 큐 처리 (FCM / APNs) · [상세](#7-push)                    |
| `smtp`       | 이메일 발송 작업 큐 적재   | `to`                                          | 비동기 큐 처리 (SMTP) · [상세](#8-smtp)                          |
| `sms`        | 문자 발송 작업 큐 적재     | `sms_receiver`, `sms_content`                 | 비동기 큐 처리 (SMS/LMS) · [상세](#9-sms)                        |
| `alimtalk`   | 카카오 알림톡 큐 적재      | `alimtalk_receiver`, `alimtalk_template_code` | 템플릿 코드 기반 발송 · [상세](#10-alimtalk)                     |
| `friendtalk` | 카카오 친구톡 큐 적재      | `friendtalk_receiver`, `friendtalk_content`   | 텍스트/이미지/캐러셀 지원 · [상세](#11-friendtalk)               |
...
- [조인](join-routes.md)
- [푸시 라우트](push-routes.md) (`push`)
- [SMTP 라우트](smtp-routes.md) (`smtp`)
- [푸시 알림 가이드](../notification/push-guide.md) (`push`)
- [SMTP 이메일 발송 가이드](../notification/smtp-guide.md) (`smtp`)
- [SMS 가이드](../notification/sms-guide.md) (`sms`)
- [카카오 알림톡 가이드](../notification/alimtalk-guide.md) (`alimtalk`)
- [친구톡(FriendTalk) 가이드](../notification/friendtalk-guide.md) (`friendtalk`)
