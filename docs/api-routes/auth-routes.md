...
| `POST` | `/v1/auth/refresh`<br>`/v1/auth/token_refresh` | Access Token 재발급 (두 경로는 동일) |
| `POST` | `/v1/auth/logout`        | 로그아웃 (세션 무효화 — Access·Refresh 동시 거부) |
...
> `POST /v1/auth/token_refresh`는 이 라우트의 별칭으로, 동일한 핸들러로 처리됩니다.
...
세션(`session_id`)을 서버 측 블랙리스트에 등록하여 무효화합니다.
Access Token과 Refresh Token은 동일한 `session_id`를 공유하므로, 로그아웃 시
**Access Token도 만료(기본 1시간) 전에 즉시 거부**되고 해당 Refresh Token으로도 재발급이 불가능합니다.
(검증 시 Access·Refresh 모두 `session_id` 블랙리스트를 확인합니다. 구버전 `session_id` 없는 토큰은 Refresh `jti`로 폴백합니다.)

> **블랙리스트 구현**: 인메모리 저장소를 사용합니다. 서버 재시작 시 초기화됩니다(다중 인스턴스는 외부 스토어 권장).
