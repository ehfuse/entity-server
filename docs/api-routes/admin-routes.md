...
> **앱서버 플러그인 활용**: 각 플러그인이 `entities/*.json`을 가지고 있으면 서버 시작 시 `POST /create`를 호출하여 자동 등록합니다. 이 멱등 동작 덕분에 매 시작마다 안전하게 호출할 수 있습니다.
...
- [이메일 인증 라우트](../../packages/entity-app-server/docs/routes/email-verification.md)
