...
2. 해당 그룹 + 엔티티 이름으로 설정 파일이 이미 존재하는지 확인합니다 (중복 차단).
...
```json
{ "ok": false, "message": "이미 존재하는 엔티티입니다: product" }
...
- [이메일 인증 라우트](email-verification-routes.md)
- [파일 라우트](files-routes.md)
- [SMTP 라우트](smtp-routes.md)
- [SMS 라우트](sms-routes.md)
- [푸시 라우트](push-routes.md)
- [알림톡·친구톡 라우트](alimtalk-routes.md)
- [본인인증 라우트](identity-routes.md)
- [PG 결제 라우트](pg-routes.md)
