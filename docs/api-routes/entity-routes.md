...
- 공통 인증 헤더 및 `skipHooks` 사용법은 [API 라우트](api-routes.md)를 참조하세요.
...
| 9.5 | [배치 생성/수정](#entity-batch-submit)                     | `POST`     | `/v1/entity/:entity/batch-submit`          |
        ...
        "items": [
            {
                ...
                "changed_by": 2,
                "changed_time": "2026-01-01 12:00:00"
            },
            {
                "seq": 15,
                "action": "UPDATE",
                "data_snapshot": { "name": "홍길동", "email": "hong@example.com" },
                "changed_by": 2,
                "changed_time": "2026-01-02 09:30:00"
            }
        ]
    }
}
...
>   상세 내용은 [History · Revision · Rollback](../data/history-revision-guide.md)를 참조하세요.
...
<a id="entity-batch-submit"></a>

### 9.5. 배치 생성/수정

여러 엔티티 레코드를 한 번의 요청으로 일괄 생성/수정합니다. 각 항목은 단건 `submit`과 동일한 Upsert 규칙을 따릅니다.

**엔드포인트**: `POST /v1/entity/:entity/batch-submit`
...
```json
{
    "items": [
        ...
        { "seq": 1, "name": "홍길순", "email": "hong@example.com" }
    ]
}
```
...
```json
{
    ...
    "results": [
        { "ok": true, "seq": 2 },
        { "ok": false, "error": "required field missing: email" }
    ],
    "success_count": 1,
    "failure_count": 1
}
...
> **참고**: 개별 항목의 실패가 다른 항목에 영향을 주지 않습니다. 최상위 `ok`는 요청 자체의 성공 여부를 나타내고, 각 항목의 처리 결과는 `results` 배열에서 확인합니다.
...
SQL 경로에서는 **단일 트랜잭션 all-or-nothing**으로 처리되어, 복원 대상 중 하나라도 복원할 수 없으면 롤백 전체가 실패(500)하고 아무것도 변경되지 않습니다.

> 참고: 큐에 남아 있는(아직 commit 전) 트랜잭션은 단순히 큐를 폐기합니다. 이 엔드포인트의 history 기반 보상 롤백은 이미 commit된(또는 구버전 saga) 트랜잭션에 적용됩니다.
...
> 롤백 메커니즘 상세는 [History · Revision · Rollback](../data/history-revision-guide.md)를 참조하세요.
