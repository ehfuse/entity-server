...
- 스토리지 설정, 썸네일, 쿼터 등 운영 가이드는 [storage-guide.md](../guides/extensions/storage-guide.md)를 참조하세요.
...
> 모든 파일 API는 `storage.json`이 설정되어 있어야 사용 가능합니다. 미설정 시 `500` 에러를 반환합니다.
...
> 지원 형식:
>
> - IANA 타임존: [timezone-list.md](../guides/data/timezone-list.md)
...
>
> 상세 규칙은 [시스템 엔티티 가이드 — 암호화 키 결정 규칙](../guides/data/system-entities.md#8-암호화-키-결정-규칙)을 참조하세요.
