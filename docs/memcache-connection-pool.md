...
캐시 저장소(`MemcacheStore`)와 Nonce 저장소(`MemcacheNonceStore`)는 각각 **독립적인 `ConnPool` 인스턴스**를 생성합니다.
`maxIdle` 설정이 두 곳에 각각 적용되며, 커넥션을 서로 공유하지 않습니다.
...
- `ConnPool.Close()`를 호출하면 모든 유휴 커넥션을 닫고 `closed = true`로 표시
- 이후 `Put()`은 커넥션을 반환하지 않고 즉시 닫음
...
Memcache 키는 공백 문자를 포함할 수 없습니다. 두 저장소 모두 내부적으로 키를 자동 정제합니다.

| 저장소               | 처리 규칙                                                  |
| -------------------- | ---------------------------------------------------------- |
| `MemcacheStore`      | 공백 → `_` 치환, 200자 초과 시 잘림                        |
| `MemcacheNonceStore` | 공백 → `_` 치환, 250자 초과 시 잘림 (`nonce:` prefix 포함) |

키 앞에 prefix를 붙이거나 긴 값을 사용할 경우 이 제한을 고려해야 합니다.
